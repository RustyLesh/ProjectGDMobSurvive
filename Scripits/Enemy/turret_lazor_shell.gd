class_name TurretLazorShell extends EnemyShell
...
func init_enemy(_enemy_resource: EnemyResource):
	...
	lazor_animation_player.play("RESET")
	#Resource setup
	...
	character_body.init_ai(enemy_resource)
	lazor.damage = _enemy_resource.lazor_damage
	#Spawn
